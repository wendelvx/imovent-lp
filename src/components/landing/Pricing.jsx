...
import { Zap, ShieldCheck, Plus, CheckSquare, Wallet } from 'lucide-react';
...
export default function Pricing() {
  return (
    <section className="bg-slate-50 py-32 px-6 lg:px-12 border-y-2 border-slate-900 relative">
      ...
      <div className="max-w-4xl mx-auto text-center mb-20 md:mb-24">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-indigo-50 border-2 border-indigo-200 shadow-[2px_2px_0px_0px_rgba(79,70,229,0.2)] mb-6">
            ...
            <span className="text-[11px] font-black text-indigo-900 tracking-widest uppercase">Investimento que se paga</span>
          </div>
          <h2 className="text-4xl md:text-5xl lg:text-[4rem] font-extrabold text-slate-950 mb-6 tracking-tighter leading-[1.05]">
            O sistema que custa menos que <span className="text-indigo-600">uma venda perdida.</span>
          </h2>
          <p className="text-lg text-slate-600 font-medium px-4 max-w-2xl mx-auto leading-relaxed">
            Pare de pagar por várias ferramentas que não se conversam. Escolha a infraestrutura que cresce junto com o seu volume de vendas, sem letras miúdas ou fidelidade.
          </p>
        </motion.div>
      </div>
      ...
      <motion.div variants={staggerContainer} initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-50px" }} className="max-w-5xl mx-auto grid md:grid-cols-2 gap-y-12 md:gap-y-0 md:gap-x-12 items-stretch">
        ...
        <motion.div variants={fadeInUp} className="bg-white border-2 border-slate-900 p-8 shadow-[6px_6px_0px_0px_rgba(15,23,42,1)] flex flex-col h-full group hover:-translate-y-1 transition-transform rounded-2xl">
          <div className="mb-6 border-b-2 border-slate-100 pb-6">
            ...
            <p className="text-sm text-slate-600 font-bold uppercase tracking-wide bg-slate-100 inline-block px-2 py-1 border border-slate-200">
              Sua Vitrine Digital
            </p>
          </div>
          ...
          <div className="mb-8 text-[11px] font-black text-emerald-700 bg-emerald-50 inline-block px-3 py-1.5 border border-emerald-200 self-start uppercase tracking-wider">
            Ideal para quem está começando ou atua sozinho.
          </div>
          ...
          <div className="mt-auto">
            <div className="p-5 bg-slate-50 border-2 border-slate-200 mb-6 flex flex-col gap-2 rounded-xl">
              ...
              <div className="flex justify-between items-center text-xs font-bold text-slate-700 border-b border-slate-200 pb-2">
                <span>+1 Corretor na conta</span> <span className="text-slate-950 font-black">R$ 59,90</span>
              </div>
              <div className="flex justify-between items-center text-xs font-bold text-slate-700 border-b border-slate-200 pb-2">
        ...
        <motion.div variants={fadeInUp} className="bg-slate-950 border-2 border-indigo-500 p-8 shadow-[6px_6px_0px_0px_rgba(79,70,229,1)] flex flex-col h-full relative z-10 transform md:-translate-y-4 rounded-2xl">
          ...
          <div className="mb-6 mt-2 md:mt-0 border-b-2 border-slate-800 pb-6">
            ...
            <p className="text-sm text-indigo-300 font-bold uppercase tracking-wide bg-indigo-950 border border-indigo-800 inline-block px-2 py-1">
          ...
          <div className="mb-8 text-[11px] font-black text-emerald-400 bg-emerald-950/50 inline-block px-3 py-1.5 border border-emerald-800 self-start uppercase tracking-wider">
            Sua imobiliária organizada e seus dados protegidos.
          </div>
          ...
          <div className="mt-auto">
            <div className="p-5 bg-[#0B0F19] border-2 border-slate-800 mb-6 flex flex-col gap-2 rounded-xl">
              ...
              <div className="flex justify-between items-center text-xs font-bold text-slate-400 border-b border-slate-800 pb-2">
                <span>+1 Novo Usuário</span> <span className="text-white font-black">R$ 29,90</span>
              </div>
              <div className="flex justify-between items-center text-xs font-bold text-slate-400 border-b border-slate-800 pb-2">
      ...
      <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ delay: 0.4 }} className="mt-20 text-center flex flex-col sm:flex-row items-center justify-center gap-3 text-slate-600 font-bold uppercase tracking-wide text-xs px-4">
        <ShieldCheck className="w-5 h-5 text-emerald-600 flex-shrink-0" />
        <p>Pagamento Seguro. Cancele sua assinatura quando quiser, sem burocracia.</p>
      </motion.div>
